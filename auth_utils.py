...
class AuthJwtCsrf():
    ...
    def verify_csrf_update_jwt(self, request, csrf_project, headers) -> str:
        csrf_token = csrf_project.get_csrf_from_headers(headers)
        csrf_project.validate_scrf(csrf_token)
        subject = self.verify_jwt(request)
        new_token = self.encode_jwt(subject)
        return new_token